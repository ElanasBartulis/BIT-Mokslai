  ...
  istrinti() {
    ...
    tableBody.addEventListener("click", (event) => {
      if (event.target.classList.contains("delete")) {
        ...
        visosKategorijos.forEach((value, index) => {
          value.filter((x) => {
            if (x.id === runnerId) {
              kurisIndex = index;
            }
        ...
        if (kurisIndex === 0) {
          this.#IkategorijosBegikai.forEach((value, index) => {
            if (value.id === runnerId) {
              this.#IkategorijosBegikai.splice(index, 1);
              console.log(this.#IkategorijosBegikai);
            }
          });
        } else if (kurisIndex === 1) {
          this.#IIkategorijosBegikai.forEach((value, index) => {
            if (value.id === runnerId) {
              this.#IIkategorijosBegikai.splice(index, 1);
              console.log(this.#IIkategorijosBegikai);
            }
          });
        } else if (kurisIndex === 2) {
          this.#IIIkategorijosBegikai.forEach((value, index) => {
            if (value.id === runnerId) {
              this.#IIIkategorijosBegikai.splice(index, 1);
              console.log(this.#IIIkategorijosBegikai);
            }
          });
        }
